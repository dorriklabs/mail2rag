...
class StateManager:
    ...
    def _generate_unique_secure_id(self, state):
        ...
        self.logger.warning("Collisions répétées sur secure_id, génération d'un ID plus long.")
    ...
    def get_or_create_secure_id(self, state, uid):
        ...
        # S'assurer de la présence de la structure
        if 'secure_ids' not in state or not isinstance(state['secure_ids'], dict):
            state['secure_ids'] = {}
        ...
        # Nouveau secure_id totalement opaque (ne contient plus l'UID)
